...
new
#[Layout('layouts.app')]
#[Title('Station Spot Inspections')]
class extends Component
{
    ...
    public function dailyStats(): array
    {
        ...
        $base = InspectionRecord::query()
            ...
            ->whereHas('workStation', fn (Builder $q) => $q->where('type', WorkStationType::StationSpot))
        ...
        $ok = (clone $base)->whereHas('stationSpotDetails', fn (Builder $q) => $q->where('auto_judgement', JudgementResult::Ok))->count();
        $ng = (clone $base)->whereHas('stationSpotDetails', fn (Builder $q) => $q->where('auto_judgement', JudgementResult::Ng))->count();
        ...
        $dayOk = (clone $dayBase)->whereHas('stationSpotDetails', fn (Builder $q) => $q->where('auto_judgement', JudgementResult::Ok))->count();
        $nightOk = (clone $nightBase)->whereHas('stationSpotDetails', fn (Builder $q) => $q->where('auto_judgement', JudgementResult::Ok))->count();
        $dayNg = (clone $dayBase)->whereHas('stationSpotDetails', fn (Builder $q) => $q->where('auto_judgement', JudgementResult::Ng))->count();
        $nightNg = (clone $nightBase)->whereHas('stationSpotDetails', fn (Builder $q) => $q->where('auto_judgement', JudgementResult::Ng))->count();
    ...
    private function overallJudgement($details): ?JudgementResult
    {
        $allOk = $details->every(fn ($d) => $d->auto_judgement === JudgementResult::Ok);

        if ($allOk) {
            return JudgementResult::Ok;
        }

        if ($details->contains(fn ($d) => $d->auto_judgement === JudgementResult::Ng)) {
    ...
    public function showStageHistory(int $partId, string $stage, string $shift): void
    {
        ...
        $records = InspectionRecord::query()
            ->with(['workStation', 'checker', 'stationSpotDetails.partHardwareMapping.hardwareType'])
        ...
        $this->stageHistory = $records->map(function ($record, $index) {
            $details = $record->stationSpotDetails;
            ...
            return [
                ...
                'overall_judgement' => $this->overallJudgement($details),
                'details' => $details->map(fn ($d) => [
                    'hardware' => $d->partHardwareMapping?->hardwareType?->part_name ?? '—',
                    'hardware_part_number' => $d->partHardwareMapping?->hardwareType?->part_number ?? '—',
                    'measurement_value' => $d->measurement_value,
                    'auto_judgement' => $d->auto_judgement,
                ])->all(),
            ];
        })->all();
    ...
    public function partsList(): LengthAwarePaginator
    {
        ...
        return Part::query()
            ->whereHas('stationTypes', fn (Builder $q) => $q->where('work_station_type', WorkStationType::StationSpot->value))
            ->with(['inspectionRecords' => function ($query) use ($targetDate) {
                $query->whereDate('production_date', $targetDate)
                    ->whereHas('workStation', fn (Builder $q) => $q->where('type', WorkStationType::StationSpot))
                    ->with(['workStation', 'stationSpotDetails'])
                    ->orderBy('stage');
            }])
            ->when($this->search, function (Builder $q) {
...
<div>
    <x-header title="Station Spot Inspection Board" subtitle="Daily quality overview for Station Spot welding lines." separator progress-indicator>
                            ...
                            @foreach ($stageKeys as $stageKey => $label)
                                @php
                                    ...
                                    if ($totalInspections > 0) {
                                        ...
                                        $allDetails = $latestRecord->stationSpotDetails;
                                        $hasAnyNg = $allDetails->contains(fn($d) => $d->auto_judgement === JudgementResult::Ng);
                                        $allOk = $allDetails->isNotEmpty() && $allDetails->every(fn($d) => $d->auto_judgement === JudgementResult::Ok);
                ...
                <div class="relative max-h-[50vh] md:max-h-[60vh] overflow-y-auto pr-1 md:pr-2">
                    ...
                    <div class="space-y-3 md:space-y-4">
                        @foreach($stageHistory as $history)
                            @php $judgement = $history['overall_judgement']; @endphp
                            <div class="relative pl-8 md:pl-10">
                                <span @class([
                                    ...
                                    'bg-success' => $judgement === JudgementResult::Ok,
                                    'bg-error' => $judgement === JudgementResult::Ng,
                                ])>
                                    #{{ $history['attempt'] }}
                                </span>
                                ...
                                <div @class([
                                    ...
                                    'border-success/30 bg-success/5' => $judgement === JudgementResult::Ok,
                                    'border-error/30 bg-error/5' => $judgement === JudgementResult::Ng,
                                ])>
                                    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-base-200 pb-2 md:pb-3 mb-2 md:mb-3">
                                        ...
                                        <div class="flex items-center gap-1.5 md:gap-2 flex-wrap">
                                            ...
                                            <x-badge
                                                ...
                                                class="{{ $judgement?->badgeClass() ?? 'badge-ghost' }} badge-xs md:badge-sm text-white font-bold"
                                            />
                                        </div>
                                    </div>
                                        ...
                                        <table class="table table-compact w-full text-[10px] md:text-xs">
                                            <thead>
                                                <tr class="bg-base-200/50 text-base-content/85">
                                                    <th class="py-1.5 md:py-2">Hardware</th>
                                                    <th class="text-center py-1.5 md:py-2">Measurement</th>
                                            ...
                                            <tbody>
                                                @forelse ($history['details'] as $detail)
                                                    <tr class="hover:bg-base-200/30">
                                                        <td class="font-semibold py-1.5 md:py-2">
                                                            {{ $detail['hardware'] }}
                                                            <span class="font-mono text-base-content/50">({{ $detail['hardware_part_number'] }})</span>
                                                        </td>
                                                        <td class="text-center py-1.5 md:py-2 font-mono">
                                                            {{ number_format($detail['measurement_value'], 2) }}
                                                        </td>
                                                        <td class="text-center py-1.5 md:py-2">
                                                            <x-badge
                                                                :value="$detail['auto_judgement']?->label() ?? '—'"
                                                                class="{{ $detail['auto_judgement']?->badgeClass() ?? 'badge-ghost' }} badge-xs text-white font-bold"
                                                            />
                                                        </td>
                                                    </tr>
                                                @empty
                                                    <tr>
                                                        <td colspan="3" class="text-center text-base-content/40 py-4">No measurement details recorded.</td>
                                                    </tr>
                                                @endforelse
                                            </tbody>
                                        </table>
