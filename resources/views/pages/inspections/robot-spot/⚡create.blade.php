...
new
#[Layout('layouts.app')]
#[Title('New Robot Spot Inspection')]
class extends Component {
    ...
    public ?string $jigOk = null;

    public string $remarks = '';

    public string $weldLength = '';
    ...
    public function needsRecheckConfirmation(): bool
    {
        ...
        return InspectionRecord::where('part_id', $this->partId)
            ...
            ->whereHas('robotSpotDetail', fn ($q) => $q->where('auto_judgement', JudgementResult::Ok))
            ->exists();
    }
    ...
    public function partHistory(): ?array
    {
        ...
        $records = InspectionRecord::where('part_id', $this->partId)
            ->whereHas('workStation', fn ($q) => $q->where('type', WorkStationType::RobotSpot))
            ->with('robotSpotDetail')
            ->latest('checked_at')
            ->get();
    ...
    public function weldLengthStandard(): ?\App\Models\WeldLengthStandard
    {
        if (! $this->partId) {
            return null;
        }

        return Part::findOrFail($this->partId)->weldLengthStandard;
    }

    public function weldLengthJudgement(): ?JudgementResult
    {
        $standard = $this->weldLengthStandard();

        if (! $standard || $this->weldLength === '' || $this->weldLength === null) {
            return null;
        }

        $value = (float) $this->weldLength;

        if ($value >= $standard->min_length && $value <= $standard->max_length) {
    ...
    public function overallNg(): bool
    {
        if ($this->jigOk === '0') {
            return true;
        }

        if ($this->weldLengthJudgement() === JudgementResult::Ng) {
            return true;
        }
    ...
    public function rules(): array
    {
        return [
            ...
            'jigOk' => ['required', 'in:0,1'],
            'remarks' => ['nullable', 'string', 'max:1000'],
            'weldLength' => ['required', 'numeric'],
        ];
    }
    ...
    public function save(): void
    {
        ...
        $record = DB::transaction(function () use ($data) {
            ...
            $autoJudgement = $this->weldLengthJudgement();

            $record->robotSpotDetail()->create([
                'jig_ok' => (bool) $data['jigOk'],
                'jig_remarks' => $data['remarks'] ?: null,
                'weld_length' => (float) $data['weldLength'],
        ...
        $this->success(
            'Robot Spot inspection recorded.',
            position: 'toast-bottom',
            redirectTo: route('inspections.robot-spot.index'),
        );
    }
        ...
        return [
            ...
            'partSearchResults' => strlen($this->partSearch) >= 2
                ? Part::where(function ($q) {
                    ...
                    ->whereHas('stationTypes', fn ($q) => $q->where('work_station_type', WorkStationType::RobotSpot->value))
                    ->orderBy('part_number')
                    ->limit(8)
                    ->get()
                : collect(),
...
<div>
    <x-header title="New Robot Spot Inspection" subtitle="Weld length measurement and jig conformance for Robot Spot welding." separator>
        <x-slot:actions>
            <x-button label="Back to records" link="{{ route('inspections.robot-spot.index') }}" icon="o-arrow-left" responsive />
        </x-slot:actions>
    </x-header>
    ...
    <x-form wire:submit="save">
                ...
                @if (! $selectedPart)
                    <x-card shadow>
                        <div class="flex flex-col items-center gap-5 py-12 text-center">
                            <x-icon name="o-computer-desktop" class="h-12 w-12 text-base-content/30" />
                            <div class="max-w-sm">
                ...
                @else
                    ...
                    <div class="grid gap-6">
                            ...
                            <x-card shadow>
                                <div class="flex flex-col items-center gap-4 py-6 text-center">
                                    ...
                                    <div class="flex gap-3">
                                        <x-button label="No, go back" link="{{ route('inspections.robot-spot.index') }}" />
                        ...
                        @if (! $needsRecheck && $partHistory && $partHistory['total'] > 0)
                            <x-card title="Inspection history" subtitle="Previous inspections for this part on Robot Spot lines." shadow>
                        ...
                        {{-- Checklist form --}}
                        @if (! $needsRecheck)
                            <x-card title="Checklist" subtitle="Inspect jig conformance and measure weld length." shadow>
                                <div class="grid gap-8">
                                    ...
                                    <fieldset>
                                        ...
                                        <div class="flex flex-wrap gap-3">
                                            @foreach ([['value' => '1', 'label' => 'OK'], ['value' => '0', 'label' => 'NG']] as $opt)
                                                <label
                                                    class="flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-2.5 text-sm transition
                                                        {{ $jigOk === $opt['value'] ? 'border-primary bg-primary/5 font-medium' : 'border-base-300 hover:border-base-content/30' }}"
                                                >
                                                    <input type="radio" wire:model.live="jigOk" value="{{ $opt['value'] }}" class="radio radio-primary radio-sm" />
                                                    {{ $opt['label'] }}
                                                </label>
                                            @endforeach
                                        </div>
                                        ...
                                        <div class="flex flex-wrap items-end gap-4">
                                            <div class="w-40">
                                                <x-input
                                                    ...
                                                    type="number"
                                                    step="0.01"
                                                    wire:model.live="weldLength"
                                                    placeholder="0.00"
                                                />
                                            </div>
                                            @php
                                                $weldJudgement = $this->weldLengthJudgement();
                                            @endphp
                                            @if ($weldJudgement)
                                                <div class="pb-2">
                                                    <x-badge
                                                        :value="$weldJudgement->label()"
                                                        :class="($weldJudgement->badgeClass() ?? 'badge-ghost') . ' badge-sm text-white font-bold'"
                                                    />
                                                </div>
                                            @endif
                                            @php
                                            ...
                                            @if ($standard)
                                                <div class="pb-2 text-xs text-base-content/50">
                                                    Standard: {{ $standard->min_length }} – {{ $standard->max_length }} {{ $standard->unit }}
                                                </div>
                                            @endif
                                        </div>
                                    </fieldset>
        ...
        @if ($selectedPart && ! $needsRecheck)
            <x-slot:actions>
                <x-button label="Cancel" link="{{ route('inspections.robot-spot.index') }}" />
