...
new
#[Layout('layouts.app')]
#[Title('Portable Spot Inspections')]
class extends Component
{
    ...
    public function dailyStats(): array
    {
        ...
        $base = InspectionRecord::query()
            ...
            ->whereHas('workStation', fn (Builder $q) => $q->where('type', WorkStationType::PortableSpot))
        ...
        $ok = (clone $base)->whereHas('portableSpotDetail', fn (Builder $q) => $q->where('is_ok', true))->count();
        $ng = (clone $base)->whereHas('portableSpotDetail', fn (Builder $q) => $q->where('is_ok', false))->count();
        ...
        $dayOk = (clone $dayBase)->whereHas('portableSpotDetail', fn (Builder $q) => $q->where('is_ok', true))->count();
        $nightOk = (clone $nightBase)->whereHas('portableSpotDetail', fn (Builder $q) => $q->where('is_ok', true))->count();
        $dayNg = (clone $dayBase)->whereHas('portableSpotDetail', fn (Builder $q) => $q->where('is_ok', false))->count();
        $nightNg = (clone $nightBase)->whereHas('portableSpotDetail', fn (Builder $q) => $q->where('is_ok', false))->count();
    ...
    public function showStageHistory(int $partId, string $stage, string $shift): void
    {
        ...
        $records = InspectionRecord::query()
            ->with(['workStation', 'checker', 'portableSpotDetail'])
        ...
        $this->stageHistory = $records->map(function ($record, $index) {
            $detail = $record->portableSpotDetail;
    ...
    public function partsList(): LengthAwarePaginator
    {
        ...
        return Part::query()
            ->whereHas('stationTypes', fn (Builder $q) => $q->where('work_station_type', WorkStationType::PortableSpot->value))
            ->with(['inspectionRecords' => function ($query) use ($targetDate) {
                $query->whereDate('production_date', $targetDate)
                    ->whereHas('workStation', fn (Builder $q) => $q->where('type', WorkStationType::PortableSpot))
                    ->with(['workStation', 'portableSpotDetail'])
                    ->orderBy('stage');
            }])
            ->when($this->search, function (Builder $q) {
...
<div>
    <x-header title="Portable Spot Inspection Board" subtitle="Daily quality overview for Portable Spot welding lines." separator progress-indicator>
    ...
    <x-card shadow class="border border-base-200">
        <div class="flex flex-wrap items-center justify-between gap-3 pb-4 mb-1 border-b border-base-200">
            ...
            <div class="flex items-center gap-3 text-xs text-base-content/60">
                <span class="flex items-center gap-1.5 rounded-full bg-base-200 px-2.5 py-1"><span class="w-2 h-2 rounded-full bg-success"></span> Pass</span>
                <span class="flex items-center gap-1.5 rounded-full bg-base-200 px-2.5 py-1"><span class="w-2 h-2 rounded-full bg-error"></span> Fail</span>
                            ...
                            @foreach ($stageKeys as $stageKey => $label)
                                @php
                                    ...
                                    if ($totalInspections > 0) {
                                        ...
                                        $isOk = $latestRecord->portableSpotDetail?->is_ok;

                                        if ($isOk === true) {
                                            $badgeColor = 'btn-success text-white hover:bg-success-focus';
                                            $tooltip = "Pass ({$totalInspections}x inspected)";
                                        } elseif ($isOk === false) {
                                            $badgeColor = 'btn-error text-white hover:bg-error-focus';
                                            $tooltip = "Fail ({$totalInspections}x inspected)";
                                        } else {
                        ...
                        @foreach($stageHistory as $history)
                            <div class="relative pl-8 md:pl-10">
                                <span @class([
                                    ...
                                    'bg-success' => $history['is_ok'] === true,
                                    'bg-error' => $history['is_ok'] === false,
                                ])>
                                    #{{ $history['attempt'] }}
                                </span>
                                ...
                                <div @class([
                                    ...
                                    'border-success/30 bg-success/5' => $history['is_ok'] === true,
                                    'border-error/30 bg-error/5' => $history['is_ok'] === false,
                                ])>
                                    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-base-200 pb-2 md:pb-3 mb-2 md:mb-3">
                                        ...
                                        <div class="flex items-center gap-1.5 md:gap-2 flex-wrap">
                                            ...
                                            @php
                                                $psLabel = $history['is_ok'] === true ? 'Pass' : ($history['is_ok'] === false ? 'Fail' : '—');
                                                $psClass = $history['is_ok'] === true ? 'badge-success' : ($history['is_ok'] === false ? 'badge-error' : 'badge-ghost');
                                            @endphp
                                            <x-badge :value="$psLabel" class="{{ $psClass }} badge-xs md:badge-sm text-white font-bold" />
                                        </div>
                                    </div>
                                        ...
                                        <table class="table table-compact w-full text-[10px] md:text-xs">
                                            <thead>
                                                <tr class="bg-base-200/50 text-base-content/85">
                                                    ...
                                                    <th class="py-1.5 md:py-2">Remarks</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr class="hover:bg-base-200/30">
                                                    <td class="font-semibold py-1.5 md:py-2">Hammer &amp; Chisel Tap Test</td>
                                                    <td class="text-center py-1.5 md:py-2">
                                                        <x-badge
                                                            :value="$history['is_ok'] === true ? 'Pass' : ($history['is_ok'] === false ? 'Fail' : '—')"
                                                            class="{{ $history['is_ok'] === true ? 'badge-success' : ($history['is_ok'] === false ? 'badge-error' : 'badge-ghost') }} badge-xs text-white font-bold"
                                                        />
                                                    </td>
