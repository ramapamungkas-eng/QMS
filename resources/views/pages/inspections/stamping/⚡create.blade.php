...
class extends Component {
    ...
    public string $defectRemarks = '';

    public string $jigSpecOk = '';

    public string $jigRemarks = '';

    public string $manualJudgement = '';
    ...
    public function needsRecheckConfirmation(): bool
    {
        ...
        return InspectionRecord::where('part_id', $this->partId)
            ...
            ->whereHas('stampingDetail', fn ($q) => $q->where('manual_judgement', JudgementResult::Ok))
            ->exists();
    }
    ...
    public function partHistory(): ?array
    {
        ...
        $records = InspectionRecord::where('part_id', $this->partId)
            ->whereHas('workStation', fn ($q) => $q->where('type', WorkStationType::Stamping))
            ->with('stampingDetail')
            ->latest('checked_at')
            ->get();
    ...
    public function rules(): array
    {
        return [
            ...
            'isDefect' => ['required', 'in:0,1'],
            'defectRemarks' => ['nullable', 'required_if:isDefect,1', 'string', 'max:1000'],
            'jigSpecOk' => ['required', 'in:0,1'],
            'jigRemarks' => ['nullable', 'required_if:jigSpecOk,0', 'string', 'max:1000'],
            'manualJudgement' => ['required', Rule::enum(JudgementResult::class)],
            'judgementRemarks' => ['nullable', 'required_unless:manualJudgement,ok', 'string', 'max:1000'],
        ];
    }
    ...
    public function save(): void
    {
        ...
        $record = DB::transaction(function () use ($data) {
            ...
            $record->stampingDetail()->create([
                'is_defect' => (bool) $data['isDefect'],
                'defect_remarks' => $data['defectRemarks'] ?: null,
                'jig_spec_ok' => (bool) $data['jigSpecOk'],
                'jig_remarks' => $data['jigRemarks'] ?: null,
                'manual_judgement' => $data['manualJudgement'],
                'judgement_remarks' => $data['judgementRemarks'] ?: null,
            ]);
        ...
        $this->success(
            'Stamping inspection recorded.',
            position: 'toast-bottom',
            redirectTo: route('inspections.stamping.index'),
        );
    }
        ...
        return [
            ...
            'partSearchResults' => strlen($this->partSearch) >= 2
                ? Part::where(function ($q) {
                    ...
                    ->whereHas('stationTypes', fn ($q) => $q->where('work_station_type', WorkStationType::Stamping->value))
                    ->orderBy('part_number')
                    ->limit(8)
                    ->get()
                : collect(),
...
<div>
    <x-header title="New Stamping Inspection" subtitle="Start / Middle / End checklist for Stamping lines." separator>
        <x-slot:actions>
            <x-button label="Back to records" link="{{ route('inspections.stamping.index') }}" icon="o-arrow-left" responsive />
        </x-slot:actions>
    </x-header>
    ...
    <x-form wire:submit="save">
                ...
                @else
                    ...
                    <div class="grid gap-6">
                            ...
                            <x-card shadow>
                                <div class="flex flex-col items-center gap-4 py-6 text-center">
                                    ...
                                    <div class="flex gap-3">
                                        <x-button label="No, go back" link="{{ route('inspections.stamping.index') }}" />
                        ...
                        @if (! $needsRecheck && $partHistory && $partHistory['total'] > 0)
                            <x-card title="Inspection history" subtitle="Previous inspections for this part on Stamping lines." shadow>
                                ...
                                @if ($latest = $partHistory['latest'])
                                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-base-content/60">
                                        ...
                                        <span>Latest:</span>
                                        <x-badge :value="$latest->stampingDetail?->manual_judgement->label() ?? '—'" :class="($latest->stampingDetail?->manual_judgement?->badgeClass() ?? 'badge-ghost').' badge-sm'" />
                        ...
                        {{-- Checklist form --}}
                        @if (! $needsRecheck)
                            <x-card title="Checklist" subtitle="Inspect the part and record findings." shadow>
                                <div class="grid gap-8">
                                    ...
                                    <fieldset>
                                            ...
                                            @foreach ([['value' => '0', 'label' => 'No'], ['value' => '1', 'label' => 'Yes']] as $opt)
                                                <label
                                                    class="flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-2.5 text-sm transition
                                                        {{ $isDefect === $opt['value'] ? 'border-primary bg-primary/5 font-medium' : 'border-base-300 hover:border-base-content/30' }}"
                                                >
                                                    <input type="radio" wire:model.live="isDefect" value="{{ $opt['value'] }}" class="radio radio-primary radio-sm" />
                                                    {{ $opt['label'] }}
                                                </label>
                                            @endforeach
                                        </div>
                                        @if ($isDefect === '1')
                                            <x-textarea
                                                wire:model="defectRemarks"
                                                label="Describe the defect"
                                                placeholder="What kind of defect was found?"
                                                rows="2"
                                                class="mt-3"
                                            />
                                        @endif
                                    ...
                                    <fieldset>
                                        <legend class="mb-2 flex items-center gap-2 text-sm font-medium">
                                            <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary text-xs text-primary-content">2</span>
                                            Is the jig spec OK?
                                            <span class="text-[10px] text-red-400">*required</span>
                                        </legend>
                                        <div class="flex flex-wrap gap-3">
                                            @foreach ([['value' => '1', 'label' => 'Yes'], ['value' => '0', 'label' => 'No']] as $opt)
                                                <label
                                                    class="flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-2.5 text-sm transition
                                                        {{ $jigSpecOk === $opt['value'] ? 'border-primary bg-primary/5 font-medium' : 'border-base-300 hover:border-base-content/30' }}"
                                                >
                                                    <input type="radio" wire:model.live="jigSpecOk" value="{{ $opt['value'] }}" class="radio radio-primary radio-sm" />
                                                    {{ $opt['label'] }}
                                                </label>
                                            @endforeach
                                        </div>
                                        @if ($jigSpecOk === '0')
                                            <x-textarea
                                                wire:model="jigRemarks"
                                                label="Jig remarks"
                                                placeholder="What's wrong with the jig spec?"
                                                rows="2"
                                                class="mt-3"
                                            />
                                        @endif
                                    ...
                                    <fieldset>
                                        <legend class="mb-2 flex items-center gap-2 text-sm font-medium">
                                            <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary text-xs text-primary-content">3</span>
                                            Manual judgement
                                            <span class="text-[10px] text-red-400">*required</span>
                                        </legend>
                                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                                            @foreach (JudgementResult::cases() as $option)
                                                <label
                                                    class="flex cursor-pointer flex-col gap-1 rounded-xl border p-3 text-sm transition
                                                        {{ $manualJudgement === $option->value ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'border-base-300 hover:border-base-content/30' }}"
                                                >
                                                    <div class="flex items-center gap-2">
                                                        <input type="radio" wire:model.live="manualJudgement" value="{{ $option->value }}" class="radio radio-primary radio-sm" />
                                                        <span class="font-medium">{{ $option->label() }}</span>
                                                    </div>
                                                </label>
                                            @endforeach
                                        </div>
                                        @if ($manualJudgement && $manualJudgement !== 'ok')
                                            <x-textarea
                                                wire:model="judgementRemarks"
                                                label="Judgement remarks"
                                                placeholder="Reason NG?"
                                                rows="2"
                                                class="mt-3"
                                            />
                                        @endif
        ...
        @if ($selectedPart && ! $needsRecheck)
            <x-slot:actions>
                <x-button label="Cancel" link="{{ route('inspections.stamping.index') }}" />
