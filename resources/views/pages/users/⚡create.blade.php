...
class extends Component {
    ...
    {
        $this->role = UserRole::Checker->value;
    }
    ...
    public function save(): void
    {
        ...
        // No manual Hash::make() here — password/pin are hashed automatically
        // by the 'hashed' cast on the User model.
        unset($data['password_confirmation']);

        User::create($data);

        $this->success('User created.', position: 'toast-bottom', redirectTo: route('users.index'));
    }
...
<div>
    <x-header title="New user" subtitle="Add a teammate and set up their account." separator>
        <x-slot:actions>
    ...
    <x-form wire:submit="save">
                ...
                <div class="sticky top-4 overflow-hidden rounded-2xl border border-base-300 bg-gradient-to-b from-base-200/60 to-base-100">
                    <div class="flex flex-col items-center gap-3 px-6 py-8">
                        <x-file wire:model="photo" accept="image/png, image/jpeg" class="!w-28">
                            <div class="relative">
                                <img
                                    src="{{ $photo ? $photo->temporaryUrl() : 'https://ui-avatars.com/api/?size=256&background=random&name='.urlencode($name ?: 'New User') }}"
                        ...
                        <div class="text-center">
                            <p class="text-lg font-semibold leading-tight">{{ $name ?: 'New user' }}</p>
                            <p class="font-mono text-sm text-base-content/60">{{ $nik ?: 'NIK not set yet' }}</p>
                        </div>
                    ...
                    <div class="space-y-3 border-t border-base-300 bg-base-100/60 px-6 py-5 text-sm text-base-content/70">
                        ...
                        <div class="flex items-start gap-2">
                            ...
                            <p>PIN is a 6-digit code they'll use later to authorize password changes.</p>
                        </div>
                        <div class="flex items-start gap-2">
            ...
            <div class="lg:col-span-8">
                <div class="rounded-2xl border border-base-300 bg-base-100">
                    ...
                    <section class="px-6 py-6">
                        ...
                        <p class="text-sm text-base-content/60">Set an initial password and PIN — they can change the password later using the PIN.</p>
                        <div class="mt-4 grid gap-4 sm:grid-cols-2">
                            <x-password label="Password" wire:model="password" />
                            <x-password label="Confirm password" wire:model="password_confirmation" />
                            <x-input label="PIN" wire:model="pin" icon="o-key" maxlength="6" inputmode="numeric" class="sm:col-span-2" />
                        </div>
                    </section>
                </div>
            </div>
        ...
        <x-slot:actions>
            ...
            <x-button label="Create user" icon="o-check" class="btn-primary" type="submit" spinner="save" />
        </x-slot:actions>
    </x-form>
</div>